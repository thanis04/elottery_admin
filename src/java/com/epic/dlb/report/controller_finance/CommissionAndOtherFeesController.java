...
import com.epic.dlb.report.builder.CommissionAndOtherFeesBuilder;
 ...
 */
@Controller
@RequestMapping("commission_other_fees_report")
public class CommissionAndOtherFeesController {

    @Autowired
    private UserService userService;

    @Autowired
    private CommissionAndOtherFeesBuilder commissionAndOtherFeesBuilder;
    ...
    public String showPage(HttpSession session, Model model) {
        ...
        List checkAuthorization = userService.checkAuthorization(session, "/commission_other_fees_report/show_page.htm");
        boolean authStatus = (boolean) checkAuthorization.get(1);
          ...
          if (authStatus) {
            ...
            return "pages/rpt_commission_other_fees";
        } else {
            //Authorization fail
    ...
    public String showReportData(HttpSession session,
            ...
            @RequestParam("lottery") String product) throws ParseException {
...
//                ticketsByDate = ticketReportService.getTicketByDateUsingTicket(dateFormat2.format(dateFormat.parse(fromDate)), dateFormat2.format(dateFormat.parse(toDate)));
                reportResult = commissionAndOtherFeesBuilder.commissionAndOtherFeesReportData(dateFormat.parse(fromDate), dateFormat.parse(toDate), product);
                obj.put("search_result", reportResult);
                System.out.println(reportResult);
                msg = SystemVarList.SUCCESS;
                status = true;
            } catch (Exception ex) {
                java.util.logging.Logger.getLogger(CommissionAndOtherFeesController.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            //Authorization fail
            //set response           
    ...
    public void search(HttpSession session,
        ...
        File generateExcel = commissionAndOtherFeesBuilder.generateExcel(dateFormat.parse(fromDate), dateFormat.parse(toDate),lottery);
