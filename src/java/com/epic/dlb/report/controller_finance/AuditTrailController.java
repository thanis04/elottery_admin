...
import com.epic.dlb.report.builder.AudittrailBuilder;
...
public class AuditTrailController {
    ...
    public String showPage(HttpSession session, Model model) {
        ...
        List checkAuthorization = userService.checkAuthorization(session, "/audit_trail/show_page.htm");
        boolean authStatus = (boolean) checkAuthorization.get(1);
        
          if (authStatus) {
     
            return "pages/rpt_audit_trail";
        } else {
            //Authorization fail
    ...
    public String showReportData(HttpSession session,
        ...
        if (authStatus) {
            try {
                ...
                reportResult = audittrailBuilder.getAudittrail(fromDate, toDate);
                obj.put("search_result", reportResult);
                System.out.println(reportResult);
                msg = SystemVarList.SUCCESS;
                status = true;
            } catch (Exception ex) {
                java.util.logging.Logger.getLogger(AuditTrailController.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            //Authorization fail
            //set response           
    ...
    public void search(HttpSession session,
        ...
        File generateExcel = audittrailBuilder.generateExcel(fromDate, toDate);
