 ...
 * @author nipuna_k
 */
@Component("winningTicketBuilder")
public class WinningTicketBuilder {
    ...
    public JSONArray winningTicketReportData(String fromDate, String toDate, String product) throws Exception {
        ...
        Iterator<WinningReport> list = winningFileReportService.getPurchaseHistoryForUserClaimed(fromDate, toDate, product, 0).iterator();
        ...
        while (list.hasNext()) {
            ...
            row.put("wining_date", next.getCreatedDate());
            ...
            row.put("win_amt", next.getWinningAmount());
    ...
    public File generateExcel(String fromDate, String toDate, String lottery) throws Exception {
        ...
        String tmpFilepath = Configuration.getConfiguration("TMP_STORE_DIR_PATH") + File.separator
                + "winning_report" + fromDate.replaceAll(":", "") + "_" + toDate.replaceAll(":", "") + sdf2.format(new Date()) + ".xls";

        File file = new File(tmpFilepath);

        WritableWorkbook w = Workbook.createWorkbook(file);
        WritableSheet s = w.createSheet("Winning ticket report", 0);
        ...
        s.addCell(new Label(4, 0, "Customer Reference"));
        s.addCell(new Label(5, 0, "Ticket Value"));
        s.addCell(new Label(6, 0, "Currency"));
        s.addCell(new Label(7, 0, "Winning prize"));
        s.addCell(new Label(8, 0, "Transaction Id"));

        Iterator<WinningReport> iterator = winningFileReportService
        ...
        while (iterator.hasNext()) {
            ...
            s.addCell(new Label(4, genCount, next.getFirstName() + " " + next.getLastName() + ", NIC: " + next.getNic()));
            s.addCell(new Label(5, genCount, "20"));
            s.addCell(new Label(6, genCount, "LKR"));
            s.addCell(new Label(7, genCount, next.getWinningAmount()));
            s.addCell(new Label(8, genCount, next.getTxnId()));
            genCount++;
