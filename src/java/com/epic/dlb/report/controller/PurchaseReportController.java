 ...
 */
@Controller
@RequestMapping("purchase_report")
    ...
    public String showReportData(HttpSession session,
        ...
        if (authStatus) {
            try {
                ...
                if (!lottery.equals("0")) {
                    transactionByDate = purchaseReportService.getTransactionByDate(fromDate, toDate, lottery);
                } else {
                    transactionByDate = purchaseReportService.getTransactionByDate(fromDate, toDate);
                }
                reportResult = purchaseReportBuilder.buildReportData(transactionByDate);
        ...
        if (authStatus) {
            try {
                ...
                if (!lottery.equals("0")) {
                    ...
                    transactionByDate = purchaseReportService.getTransactionByDate(fromDate, toDate);
                    summery = purchaseReportService.getTransactionByDate(fromDate, toDate);
                    params.put("summery", summery);
                }

                params.put("total", transactionByDate.size());

                String reportPath = "/resources/reports/purchase_report.jasper";
