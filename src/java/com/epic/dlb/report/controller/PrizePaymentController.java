...
import com.epic.dlb.report.builder.PrizePaymentBuilder;
 ...
 */
@Controller
@RequestMapping("prize_payment_report")
public class PrizePaymentController {
    ...
    @Autowired
    private PrizePaymentBuilder prizePaymentBuilder;

    @Autowired
    private PrizePaymentService paymentPaymentService;

    @RequestMapping("/show_page.htm")
    public String showPage(HttpSession session, Model model) {
        //check Authorization
        List checkAuthorization = userService.checkAuthorization(session, "/prize_payment_report/show_page.htm");
        boolean authStatus = (boolean) checkAuthorization.get(1);
        if (authStatus) {
            //Authorization ok             
            return "pages/rpt_prize_payment";
    ...
    public String showPaginatedData(HttpSession session,
        ...
        if (authStatus) {
            try {
                ...
                JSONArray data = prizePaymentBuilder.getPrizePaymentData(reportSearchCriteriaDto);
                Integer count = paymentPaymentService.getPrizePaymentDataCount(reportSearchCriteriaDto);
    ...
    public void downloadExcel(HttpSession session, HttpServletResponse resp,
        ...
        File generateExcel = prizePaymentBuilder.getGeneratePrizePaymentExcel(reportSearchCriteriaDto);
